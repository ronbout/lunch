...
var iosFlg = (navigator.platform.indexOf("iPhone") != -1) ||
	...
	else if (iosFlg) 
	{
		...
		else
		{
			...
			if (window.innerHeight < window.innerWidth)
				...
				document.body.style.height = (window.screen.height - 64) + "px";
		}
	}
	if (androidFlg && (window.orientation == "90" || window.orientation == "-90"))
	{
	...
	switch(newPg)
	{
		...
		default:
			if (oldPg == 1)
				...
				ajaxLoadHTML("rest_pg", "ajax/restaurants_ajax.php", args, newPg, oldPg, "");
			break;
		case 1:
			if (oldPg == 2)
				preparePg(2,1,false);
			else
				ajaxLoadHTML("cats_pg", "ajax/categories_ajax.php", args, newPg, oldPg, "");
			break;
		case 2:
...
//alert(responseText);
		...
		else
		{
			...
			e.innerHTML = "";
			if (moreFlg)
				e.innerHTML = rtSplit[0] + footerMenu + slideMenu;
			else
				e.innerHTML = responseText + footerMenu + slideMenu;
...
function loadTitle(titleName)
{
	...
	if (titleName.length > 18)
		hdrH1.style.fontSize = "16px";
	else
		hdrH1.style.fontSize = "20px";
...
function loadLiLinks(e)
{
	...
	for(var i=0; i < lis.length; i++)
		if (lis[i].className.indexOf("li_link") > -1)
		{
			...
			if (aType == 1)
			{
				lis[i].setAttribute("onclick", "loadRest(\'"+hrf+"\',\'"+lis[i].innerHTML+"\')");
			}
			else
				lis[i].setAttribute("onclick", "window.open(\'"+hrf+"\')");
		}
}
			...
			if (iosFlg)
			{
				if (!window.navigator.standalone)
				{
					...
					if (window.innerHeight < window.innerWidth)
					{
						...
						rp.style.height = window.screen.height - 104 + "px";
				}
				else
				{
...
//alert("div_rest_list: " + rdh+"\nul_rest: "+ruh);
			...
			if (iosFlg)
			{
				if (!window.navigator.standalone)
				{
					if (window.innerHeight < window.innerWidth)
					{
						...
						cp.style.height = window.screen.height - 104 + "px";
				}
				else
				{
			...
			if (iosFlg)
			{
				if (!window.navigator.standalone)
				{
					if (window.innerHeight < window.innerWidth)
					{
						...
						fp.style.height = window.screen.height - 104 + "px";
				}
				else
				{
	...
	if (nutDiv != undefined && window.getComputedStyle(nutDiv,"").display == "block") 
	...
	if (helpDiv != undefined && window.getComputedStyle(helpDiv,"").display == "block") 
	...
	if (sortDiv != undefined && window.getComputedStyle(sortDiv,"").display == "block") 
...
function setSizeChckbox()
{
	...
	for (var i = 0; i < lis.length; i++)
	{
		...
		if (txtLi.innerHTML.length >= 42)
			txtLi.style.fontSize = "12px";
...
function backPage(newPg, oldPg)
{
	...
	thisBut.style[vendor + "BorderImage"] = "url(\'../images/back_button.png\') 0 8 0 8";
	if (menuFlg || srchFlg) return;
...
function liCatsClick(catId, thisLi)
{
	...
	{
		thisCheck.className = thisCheck.className.replace("li_checked", "li_unchecked");
		if (catId in catsObj) delete catsObj[catId];
	}
	else
	{
		thisCheck.className = thisCheck.className.replace("li_unchecked", "li_checked");
...
//showLocs();
		...
		case "menu_home_f":
			...
			window.open("http://ronboutilier.com", "_self");
			break;
		case "menu_loadfoods":
...
function getSortBox()
{
	...
	for (var i=0; i<sortFlds.length; i++)
	{
		...
		if (sortBy == i) 
			sortBox += ' class="radio_on" ';
		else
			sortBox += ' class="radio_off" ';
		sortBox += 'ontouchstart="radioPressed(this,'+i+')" ontouchend="radioReleased(this,'+i+')" ';
		sortBox += ' onmousedown="radioPressed(this,'+i+')" onmouseup="radioReleased(this,'+i+')" ';
		sortBox += '>'+sortFlds[i].display + '</li>';
	}
	sortBox += '</ul></div></div>\
		<div class="div_close">\
			<button class="close_but" onclick="hideSort('+sortBy+');">Close</button>\
		</div>';
	return sortBox;
}
...
{
	if (menuFlg || srchFlg) return;
	thisBut.style[vendor + "BorderImage"] = "url(\'../images/back_button_clicked.png\') 0 8 0 8";
}
...
function liPressed(thisLi)
{
	...
	if (vendor == "webkit")
		gradientCode = "-webkit-gradient(linear, left top, left bottom, from(#ccc), to(#aaa))";
	else
	...
	if (chkDiv.className.indexOf("li_unchecked") > -1)
		chkDiv.className = chkDiv.className.replace("li_unchecked", "li_uncheck_pressed");
	else
		chkDiv.className = chkDiv.className.replace("li_checked", "li_check_pressed");
	
		txtDiv.style.backgroundImage = gradientCode;
}
function liReleased(thisLi)
{
	...
	if (chkDiv.className.indexOf("li_uncheck_pressed") > -1)
		chkDiv.className = chkDiv.className.replace("li_uncheck_pressed", "li_unchecked");
	else
		chkDiv.className = chkDiv.className.replace("li_check_pressed", "li_checked");
...
{
	if (sortI == sortBy)
		thisLi.className = " radio_on_pressed ";
	else
		thisLi.className = " radio_off_pressed ";
}
function radioReleased(thisLi, sortI)
{
	if (sortI == sortBy)
		thisLi.className = " radio_on ";
	else
		thisLi.className = " radio_off ";
}
function menuPressed(thisMenu)
{
	thisMenu.style.backgroundColor = "gray";
	if (thisMenu.id == "menu_loadfoods") document.getElementById("menu_loadfoods_arrow").style.backgroundColor = "gray";
	if (thisMenu.id == "menu_loadfoods_arrow") document.getElementById("menu_loadfoods").style.backgroundColor = "gray";
}
function menuReleased(thisMenu)
{
	thisMenu.style.backgroundColor = "black";
	if (thisMenu.id == "menu_loadfoods") document.getElementById("menu_loadfoods_arrow").style.backgroundColor = "black";
	if (thisMenu.id == "menu_loadfoods_arrow") document.getElementById("menu_loadfoods").style.backgroundColor = "black";
}
...
function showLocs()
{
	...
	alert("rp height: " + window.getComputedStyle(rp,"").height + "\nrp vis: "+window.getComputedStyle(rp,"").visibility);
	alert("drl height: " + window.getComputedStyle(drl,"").height + "\ndrl top: "+window.getComputedStyle(drl,"").top);
}
...
function showDebug()
{
	for(tm in debugObj)
	{
		...
		outStr += "\ninnerHeight: "+debugObj[tm].innerHeight+"\nouterHeight: "+debugObj[tm].outerHeight;
